   ...
   useEffect(() => {
      try {
         async function fetchData() {
            const response = await axios.get("http://localhost:8000/pengajuan?id_cust=1");
            ...
            <div className="site-layout-background card">
               ...
               <div className="data__diri">
                  {loading &&
                     pengajuan.map((pengajuan) => (
                        <div className="data__content" key={pengajuan.id_pengajuan}>
                           ...
                           <div className="data__content-pengajuan">
                              ...
                              <a href="#a">{pengajuan.bukti_ktp}</a>
                           </div>
                           <div className="data__content-pengajuan">
                              <span>Bukti Slip Gaji</span>
                              <a href="#a">{pengajuan.bukti_gaji}</a>
                           </div>
                           <div className="data__content-pengajuan">
                              <span>Status Verifikasi</span>
                              {pengajuan.status === 1 ? (
                                 <h4 style={{ color: "#1890ff" }}>Menunggu Verifikasi</h4>
                              ) : pengajuan.status === 2 ? (
                                 <h4 style={{ color: "#08c40b" }}>Terverifikasi, lengkapi persyaratan dokumen tambahan !</h4>
                              ) : (
                                 <h4 style={{ color: "#1890ff" }}>Tidak Terverifikasi</h4>
                              )}
                           </div>
                        </div>
                     ))}
               ...
               <div className="button__reset">
                  <Button size="large" className="button_2">
                     <Link to={"/customer/kpr-kelengkapan"}>Lengkapi Data</Link>
                  </Button>
               </div>
            </div>
         </Content>
